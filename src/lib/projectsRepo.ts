...
db.runSync(
    ...
    VALUES (?,?,?,?,?,?,?,?)`,
    [id, p.title, p.admin, p.organization, p.protocolpdf ?? null, p.description ?? null, now, now]
);
...
const p = db.getFirstSync<any>(`SELECT * FROM projects WHERE id=?`, [id]);
if (!p) return null;

const promptRows = db.getAllSync<any>(
    `SELECT * FROM prompts WHERE projectId=? ORDER BY orderIndex ASC`, [id]
);
...
db.runSync(
    ...
    [pid, projectId, question, nextIndex, now]
);
